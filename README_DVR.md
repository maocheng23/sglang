...
### Test Correctness
```
USE_CHAIN_SPECULATIVE_SAMPLING=1 USE_DECODE_TOPK_RENORM=1  python3 -m sglang.launch_server --model Qwen/Qwen3-30B-A3B --speculative-algorithm DECODE_VERIFY_ROLLBACK --speculative-num-steps 127 --speculative-eagle-topk 1 --speculative-num-draft-tokens 128  --max-running-requests 256  --decode-log-interval 1  --enable-deterministic-inference --disable-overlap-schedule --mem-frac 0.65 --rl-on-policy-target fsdp --tp 2
...
### Inference
```
USE_CHAIN_SPECULATIVE_SAMPLING=1 python3 -m sglang.launch_server --model Qwen/Qwen3-30B-A3B --speculative-algorithm DECODE_VERIFY_ROLLBACK --speculative-num-steps 127 --speculative-eagle-topk 1 --speculative-num-draft-tokens 128  --max-running-requests 256  --decode-log-interval 1  --enable-prefill-only-deterministic-inference --disable-overlap-schedule --mem-frac 0.65 --rl-on-policy-target fsdp --tp 2
```
