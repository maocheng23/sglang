...
def _moe_sum_tree_reduce_k8_fused_kernel_opt2d(
    ...
    # h 是 tl.arange 得到的 tensor，OK
    tl.multiple_of(h, 8)          # 8/16/32 都可以试（按你希望的对齐粒度）
    tl.max_contiguous(h, BLOCK_H) # 你这行本来就是 OK 的
...
def _moe_tree_reduce_sparse_k8_kernel(
    ...
    NEG2 = -2  # sentinel: topk_ids 不可能是 -2
...
#     print("使用优化版本 moe_sum_tree_reduce_optim")
# else:
#     print("使用原始版本 moe_sum_tree_reduce_original")

if os.getenv("TREE_ALL_REDUCE_OPTIM", "0") == "1":
    tree_all_reduce_sum = tree_all_reduce_sum_optim
    print("使用优化版本 tree_all_reduce_sum_optim")
else:
    print("使用原始版本 tree_all_reduce_sum_original")
